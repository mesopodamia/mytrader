...
class EastmoneyAdapter:
    ...
    def __init__(self):
        ...
        self.base_api_url = "https://emweb.securities.eastmoney.com"
        ...
        try:
            ...
            for code in stock_codes:
                if code.startswith("6"):
                    symbols.append(f"1.{code}")  # 沪市
                else:
                    symbols.append(f"0.{code}")  # 深市
            
            url = f"{self.base_url}/api/qt/market/get"  
            params = {
            ...
            if response.status_code == 200:
                ...
                if result.get("rc") == 0:
                    ...
                    for item in result.get("data", {}).get("diff", []):
                        ...
                        if stock_code:
                            data[stock_code] = item
                    logger.info(f"获取实时行情成功: {len(data)} 只股票")
                    return data
                else:
                    logger.error(f"获取实时行情失败: {result.get('msg')}")
                    return None
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("rc") == 0:
                    ...
                    logger.info(f"获取历史数据成功: {stock_code} ({len(data)}条)")
                    return data
                else:
                    logger.error(f"获取历史数据失败: {result.get('msg')}")
                    return None
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("rc") == 0:
                    ...
                    logger.error(f"获取股票信息失败: {result.get('msg')}")
                    return None
            else:
    ...
    def search_stocks(self, keyword: str, limit: int = 10) -> Optional[List[Dict[str, Any]]]:
        ...
        """
        try:
            url = f"{self.base_api_url}/PC_HSF10/CompanySurvey/PageAjax"
            params = {
                "code": keyword,
                "type": "search",
                "count": str(limit)
            ...
            if response.status_code == 200:
                ...
                if result.get("success"):
                    ...
                    logger.error(f"搜索股票失败: {result.get('message')}")
                    return None
            else:
    ...
    def get_industry_sectors(self) -> Optional[List[Dict[str, Any]]]:
        ...
        """
        try:
            url = f"{self.base_api_url}/PC_HSF10/IndustryResearch/PageAjax"
            params = {
                "type": "industryList"
            }
            ...
            if response.status_code == 200:
                ...
                if result.get("success"):
                    ...
                    logger.error(f"获取行业板块失败: {result.get('message')}")
                    return None
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("success"):
                    ...
                    logger.error(f"获取行业股票失败: {result.get('message')}")
                    return None
            else:
...
# 便捷函数
async def get_adapter() -> EastmoneyAdapter:
    """获取东方财富网适配器实例（异步）"""
